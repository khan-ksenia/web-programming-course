...
class Repository<T extends object> {
    ...
    update(index: number, updates: Omit<Partial<T>, keyof BaseEntity>): T & BaseEntity {
        const existing = this.items[index];
        if (!existing) throw new Error(`Элемент с индексом ${index} не найден`);

        const updated: T & BaseEntity = {
            ...existing,
            ...updates,
            updatedAt: new Date(),
        };
